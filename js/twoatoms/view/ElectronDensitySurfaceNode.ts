...
export default class ElectronDensitySurfaceNode extends SurfaceNode {
  ...
  protected override updateFill(): void {
    affirm( this.colors.length === 2, 'this implementation only works for 2 colors' );
    ...
    if ( deltaEN === 0 ) {
      ...
      const gradientWidth = linear( 1, 0, surfaceWidth, surfaceWidth * MPConstants.SURFACE_GRADIENT_WIDTH_MULTIPLIER, scale );
