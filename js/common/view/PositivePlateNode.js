 ...
 * some duplicate code is better than an awkward base type.
 *
...
define( function( require ) {
   ...
   * @constructor
   */
  function PositivePlateNode( eField ) {

    Node.call( this );

    // positive polarity indicator
    var indicatorNode = new PolarityIndicator();

    var plateOptions = { fill: MPColors.POSITIVE_PLATE, stroke: 'black' };

    // side edge to show thickness
    var sideEdgeNode = new Rectangle( MPConstants.PLATE_WIDTH, 0, MPConstants.PLATE_THICKNESS, MPConstants.PLATE_HEIGHT, plateOptions );
    ...
    eField.enabledProperty.link( function( enabled ) {
      faceNode.fill = sideEdgeNode.file = ( enabled ? MPColors.POSITIVE_PLATE : MPColors.DISABLED_PLATE );
      indicatorNode.visible = enabled;
    } );
  }

  return inherit( Node, PositivePlateNode );
} );